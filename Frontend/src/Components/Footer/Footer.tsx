...
const Footer=()=>{
    ...
    return location.pathname!="/signup" && location.pathname!="/login"? <div className="pt-20 pb-5 flex gap-5 justify-around bg-mine-shaft-950 font-['poppins']">
        ...
        <div className="flex gap-1 items-center text-bright-sun-400">
            ...
            <div className="text-xl font-semibold">Hirix</div>
        </div>
        ...
        </div>
        {
            footerLinks.map((item, index) => <div key={index}>
                ...
                {
                    item.links.map((link, index) => <div key={index} className="text-mine-shaft-300 text-sm hover:text-bright-sun-400 cursor-pointer mb-1">{link}</div>)
                }
             </div> )
        }
    </div>:<></>
