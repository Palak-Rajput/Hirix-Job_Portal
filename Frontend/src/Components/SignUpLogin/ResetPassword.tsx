...
const ResetPassword=(props:any)=>{
    ...
    return <Modal opened={props.opened} onClose={props.close} title="Reset Password">
        ...
        <TextInput withAsterisk value={email} size="md" onChange={(e)=>setEmail(e.target.value)} name="email"
      leftSection={<IconAt style={{ width: rem(16), height: rem(16) }} />}
      label="Email"
      placeholder="Your email" rightSection={<Button loading={otpSending && !otpSent} size="xs" className="mr-1" onClick={handleSendOtp} autoContrast disabled={email==="" || otpSent} variant="filled">Login</Button>
    ...
    {
          <PasswordInput error={passErr} value={password} onChange={(e)=>{setPassword(e.target.value); setPassErr(signupValidation("password", e.target.value))}} name="password" withAsterisk leftSection={<IconLock style={{ width: rem(18), height: rem(18) }} stroke={1.5} />} label="Password" placeholder="Password" />}
